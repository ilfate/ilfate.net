
<div class="page-header">
  <h1>Code <small>Open-source! Feel free to use!</small></h1>
</div>

<div class="row show-grid">
  <div class="span7 offset1 code-page-block" >
    <a href="<?=Helper::url('Main', 'codeEngine')?>" class="rounded_block_link" data-target=".main-content-well">
      <?= $this->inc('interface/rounded_block.tpl', array('text' => 'Ilfate PHP framework', 'background' => '/images/php2.jpg')) ?>
    </a>
  </div>
</div>
<div class="row show-grid">
  <div class="span7 offset1 code-page-block" >
    <a href="<?=Helper::url('Main', 'codeStarred')?>" class="rounded_block_link" data-target=".main-content-well">
      <?= $this->inc('interface/rounded_block.tpl', array('text' => 'Starred label', 'background' => '/images/js3.jpg')) ?>
    </a>
  </div>
</div>


